...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Cek apakah registrasi berhasil dan tampilkan pesan success
    if (res.ok) {
      setIsSuccess(true);
      setMessage("Registration successful! Welcome!");
      setModalOpen(true);  // Tampilkan modal setelah sukses
    } else {
      setIsSuccess(false);
  ...
  const closeModal = () => {
    setModalOpen(false);  // Menutup modal ketika tombol close ditekan
  };

  return (
    <div className="m-22 bg-white border border-gray-200 rounded-xl shadow-2xs dark:bg-neutral-900 dark:border-neutral-700">
      <div className="p-4 sm:p-7">
        ...
        {/* Modal Success */}
        {modalOpen && (
          <div id="hs-task-created-alert" className="hs-overlay fixed top-0 start-0 z-80 overflow-x-hidden overflow-y-auto" role="dialog" aria-labelledby="hs-task-created-alert-label">
